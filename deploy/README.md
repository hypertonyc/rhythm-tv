...
Прежний контейнер остаётся остановленным для отката:
`docker update --restart=unless-stopped rhythm-tv && docker start rhythm-tv`
(сначала остановив `tms-server`, порт один).
