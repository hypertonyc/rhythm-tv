...
загруженные с телефона `.torrent`, там же сервер хранит `.tms-active` — файл
с id активного торрента — и `.tms-watched`, журнал просмотров, по которому
чистка места решает, какую серию выселить первой. Разово, если права
разъехались:
...
Скачанное сервер чистит сам: раз в минуту он смотрит на свободное место
и, если его меньше `STORE_MIN_FREE_GB` (на проде 10), выселяет серии — начиная
с тех, которые дольше всех не смотрели, — пока не станет
`STORE_TARGET_FREE_GB` (20). Выселенное не потеряно: куски докачаются, когда
серию снова захотят посмотреть.

```sh
# что происходит: каждое выселение пишется в лог
sudo docker logs tms-server 2>&1 | grep 'чистка места'
# журнал просмотров — по нему выбирается жертва
sudo cat /srv/rhythm-tv/data/.tms-watched
# сколько занято сейчас
df -h /; sudo du -sh /srv/rhythm-tv/cache/*
```

Свободное место и итог чистки видны и с телефона — строкой в панели
библиотеки на странице сервера.

Выключить (например, чтобы разобраться, куда делось место) —
`STORE_MIN_FREE_GB=0` в `/srv/rhythm-tv/deploy/.env` и деплой; значения
из `.env` переживают выкатки.

## Что стоит завести отдельно

Каталог сегментов HLS не ограничен по размеру by design — счётчик идёт вверх,
старые `.ts` не удаляются до конца сеанса, и чистка места его не трогает: она
умеет выселять только скачанное торрентами. Пока это не починено в сервере
(`HLS_MAX_DISK_MB`), нужен алерт по `df` на `/` (сегменты лежат
в `/srv/rhythm-tv/cache/hls`).
