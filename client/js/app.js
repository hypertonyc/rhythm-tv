...
(function () {
    ...
    function startPlayback(seconds) {
        ...
        api(path, function (err, data) {
            ...
            /* Сеанс живёт не с той секунды, которую попросили: чтобы перемотка
             * шла копированием, сервер подтягивает точку назад, на ближайший
             * ключевой кадр (в среднем полGOP, на «Друзьях» это ~2.5 с).
             * Своя цель тут больше не годится — по startOffset считается
             * и часы в HUD, и сохраняемая позиция, и сторож конца серии;
             * оставь её, и всё это будет врать на величину подтяжки в одну
             * и ту же сторону. Старый сервер поля не пришлёт — тогда остаётся
             * запрошенное, как и было. */
            if (typeof data.start === 'number' && isFinite(data.start)) startOffset = data.start;
     ...
     * нажатие — это новый сеанс ffmpeg с другим start=. С 18.08.2026 он стоит
     * секунды, а не 15-20: сервер подтягивает точку на ключевой кадр и копирует
     * поток вместо libx264 (media.SeekPoint, hls.Manager.alignSeek). Но копится
     * перемотка по-прежнему, и убирать это не надо — перезапуск не бесплатен
     * даже копированием: ffprobe за ключевым кадром, чтение куска, которого
     * может не быть на диске, и два стартовых сегмента, которых ждёт телевизор.
     *
