...
func (s *Server) probe(ctx context.Context, src mediasource.Source, index int) (*media.Result, error) {
	...
	return s.deps.Prober.Probe(ctx, media.Request{
		// Scope — путь файла в хранилище, и он же ключ кэша разбора.
		// Индексом кэш ключевать нельзя: активный торрент переключают
		// с телефона, и тот же индекс тогда означает другую серию — телевизор
		// получал имя, длительность и внешние субтитры прежнего сериала
		// поверх картинки нового. Берётся из того же снимка src, что и index,
		// поэтому переключение посреди запроса их не разводит.
		Scope: file.StorePath,
