...
const (
	...
	// joinFuse — предохранитель. Если плеер смотрит на плейлист, но так и не
	// забрал ни одного сегмента (например, эта прошивка на коротком плейлисте
	// не входит, а ждёт роста: правило запрещает вход ближе трёх
	// TARGETDURATION к концу, а подрезанный плейлист как раз короче), окно
	// всё равно открывается, и поведение вырождается в прежнее — с потерей
	// начала, но без вечного ожидания. Наблюдённых случаев такого ожидания
	// пока нет ни одного: в логе nginx это была бы серия запросов index.m3u8
	// без единого .ts между ними.
	//
	// Отмеряется от ПЕРВОГО ЗАПРОСА ПЛЕЙЛИСТА этим сеансом, а не от его старта.
	// Так пришлось сделать после наблюдения 17.08.2026: в сеансе msxf9ry6
	// (transcode, холодный рой сразу после выкатки) два стартовых сегмента
	// появились только через 33 с. Всё это время телевизор ждал в waitForHls
	// и до плейлиста добрался на 37-й секунде — а предохранитель, считавший
	// от старта сеанса, истёк семнадцатью секундами раньше. Подрезки не было
	// вовсе, прошивка применила своё правило к полному плейлисту и вошла
	// в seg00007, потеряв ~35 с серии. То есть от старта сеанса окно защищало
	// только быстрые старты (copy доходит до двух сегментов за секунду-две) —
	// ровно те, которым оно нужно меньше всего.
	//
	// Наблюдённое время от первого взгляда до выбора точки входа — 2-4 с,
	// так что 20 с это запас в пять раз.
	joinFuse = 20 * time.Second
)

// notePlayerJoined запоминает, что плеер этого сеанса уже забирал сегменты.
//
// Состояние одно на весь сервер, а не на сеанс: подрезается только активный
// сеанс, а активный ровно один — новый /api/start гасит предыдущий. Так
// не нужен ни рост карты на каждую перемотку, ни её чистка.
...
// notePlayerLooking отмечает запрос плейлиста и отвечает, держать ли окно
// подрезанным. Запись и ответ в одной функции потому, что это одна критическая
// секция: предохранитель считается от первого взгляда, и узнать, наступил ли
// он, можно только записав этот взгляд.
//
// Зовётся только для активного сеанса (см. playerPlaylist). Это важно: иначе
// оставленная открытой страница прежнего сеанса, которая всё ещё опрашивает
// его плейлист, забирала бы себе единственный слот и перевзводила
// предохранитель тому, кто в этот момент входит.
func (s *Server) notePlayerLooking(sessionID string) (holdWindow bool) {
	s.joinMu.Lock()
	defer s.joinMu.Unlock()
	if s.lookingSession != sessionID {
		s.lookingSession = sessionID
		s.lookingSince = s.now()
	}
	return s.now().Sub(s.lookingSince) <= joinFuse
}
...
func (s *Server) playerPlaylist(sessionID string, raw []byte) []byte {
	...
	// не действует, плеер и так начинает с начала.
	...
	// Подрезается только тот сеанс, который ffmpeg пишет прямо сейчас.
	//
	// Этим спасён подобранный после выкатки сеанс: его смотрят с середины,
	// и оставить в плейлисте одно начало значило бы выбить у плеера то, что он
	// играет. Одного ENDLIST для этого мало — его нет, если прежний ffmpeg
	// умер посреди серии, — а активным подобранный сеанс не становится
	// намеренно (hls/adopt.go). Прежде от этого спасал возраст сеанса
	// (подобранный всегда старше предохранителя), но возраст больше
	// не участвует вовсе, см. joinFuse.
	if snap := s.deps.HLS.ActiveSnapshot(); snap == nil || snap.ID != sessionID {
		return body
	}
	if !s.notePlayerLooking(sessionID) {
		return body
	}
