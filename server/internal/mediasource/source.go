...
//
// StorePath — путь того же файла ОТНОСИТЕЛЬНО ХРАНИЛИЩА, то есть имя раздачи
// плюс Path («Друзья/Сезон 09/s09e01 - ….mkv»). Два поля, а не одно, потому
// что это разные вещи: Path принадлежит торренту и годится для сортировки
// серий, а StorePath указывает на файл на диске и служит ключом журнала
// просмотров (internal/reclaim). Склеить одно из другого нельзя: у торрента
// из одного файла имя раздачи и есть весь путь, и склейка удвоила бы его.
type File struct {
	Index     int
	Name      string
	Path      string
